...
class DomainClusters:
    #prior_timeperiod_ms is the time, in miliseconds, to search for previous domain clusters of the same type.
    ...
    def process_vector(self, vector_id, post_id, vector):
        ...
        for term in vector:
            try:
                tl = term.lower()
                if tl in self.tag_groups.keys():
                    self.tag_groups[tl]['similar_ids'].append(vector_id)
                    self.tag_groups[tl]['similar_post_ids'].append(post_id)
                else:
