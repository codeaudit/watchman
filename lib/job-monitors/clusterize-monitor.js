...
class ClusterizeMonitor extends EventedMonitor {
  ...
  submitJobs() {
    ...
    let queryUrl = util.format('%s/socialMediaPosts? \
      filter[where][timestamp_ms][between][0]=%s& \
      filter[where][timestamp_ms][between][1]=%s& \
      filter[where][lang]=%s',
      process.env.API_ROOT,
      this.start_time, this.end_time, this.jobMonitor.lang);
    ...
    let jobAttrs = {
      ...
      start_time_ms: this.end_time,
      lang: this.jobMonitor.lang,
