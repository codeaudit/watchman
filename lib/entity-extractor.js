...
module.exports = {
  ...
  extractFeatures(archiveUrl) {
    var parsed = url.parse(archiveUrl),
      ...
      downloadFile = strf('/downloads/features/{0}-{1}', jobId, path.basename(parsed.pathname)),
  ...
  extractWithStanford(text, mimeType) {
    ...
    var filePath = strf('/tmp/stanner-{0}.txt', idGen.randomish());
    return new Promise((res, rej) => {
